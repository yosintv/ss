...
export default function App(): JSX.Element {
  ...
  const [showNewGame, setShowNewGame] = useState(initialMode === null)
    ...
    <div className="min-h-screen bg-[var(--table-bg)] text-white" style={{ backgroundImage: 'var(--table-texture)' }}>
      ...
      <NewGameModal
        ...
        onStart={(mode, seedValue, options) => {
          if (options?.daily) {
            const mapped: Difficulty = mode === 'scorpio' ? '4suit' : mode
            newGame(mapped, seedValue, { daily: true })
            setShowNewGame(false)
            return
          }
          const target = modeToPath[mode]
          const url = `${target}?seed=${encodeURIComponent(seedValue)}`
